document.addEventListener('DOMContentLoaded', async function () {
    ...
    async function fetchSurfData(lat, lng) {
        try {
            ...
            return data.hours ? data.hours[0] : null; // Ensure data structure is valid
        } catch (error) {
    ...
    function convertDegreesToDirection(degrees) {
        ...
        return degrees !== undefined ? directions[Math.round(degrees / 22.5) % 16] : "unknown";
    }

    function determineBestSurfTime(tide) {
        if (tide === "incoming") return "Good";
        if (tide === "low" || tide === "medium") return "Fair";
        return "Not Ideal";
    }
    ...
    async function updateSurfAlerts() {
        ...
        const spotData = await Promise.all(surfSpots.map(async (spot) => {
            ...
            const windDirection = conditions.windDirection ? convertDegreesToDirection(conditions.windDirection.sg || conditions.windDirection.noaa) : "undefined";
            const swellDirection = conditions.swellDirection ? convertDegreesToDirection(conditions.swellDirection.sg || conditions.swellDirection.noaa) : "undefined";
            const period = conditions.swellPeriod ? conditions.swellPeriod.sg || conditions.swellPeriod.noaa : "undefined";
            const waveHeight = conditions.waveHeight ? conditions.waveHeight.sg || conditions.waveHeight.noaa : "undefined";
            const tide = conditions.tide ? conditions.tide.sg || conditions.tide.noaa : "undefined";
            ...
            return {
                ...
                conditions: {
                    ...
                    windDirection,
                    tide,
                    bestSurfTime: determineBestSurfTime(tide)
                }
            };
        }));
        ...
        sortedSpots.forEach((spot) => {
            ...
            spotElement.innerHTML = `
                ...
                <p>🌅 Best Surf Time: ${spot.conditions.bestSurfTime}</p>
